...
### Acceptance
- [ ] 카메라 권한 거부 → "카메라 권한이 필요해요" + "설정 열기"/"취소"
- [ ] 사진 권한 거부 → "사진 권한이 필요해요" + "설정 열기"/"취소"
- [ ] "설정 열기" → `Linking.openSettings` 호출
- [ ] 권한 거부 시 addPage 미호출(빈 상태 유지)
- [ ] 단위 테스트(안내 로직) + e2e(S17) 작성·실행 · `checks.sh` PASS
