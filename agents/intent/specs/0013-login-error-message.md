...
### Acceptance
- [ ] 네트워크 오류(`fetch` 실패) → `"네트워크 연결을 확인해주세요"`
- [ ] 서버/기타 실패 → `"로그인에 실패했어요. 잠시 후 다시 시도해주세요"`
- [ ] 사용자 취소 → 문구 미표시(`error === null`), 로그인 화면 유지
- [ ] 실패 후 Capture 미진입·재시도 가능
- [ ] 단위 테스트(매핑 4케이스) + e2e(S16) 작성·실행 · `checks.sh` PASS
