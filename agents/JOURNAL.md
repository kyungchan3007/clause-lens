...
- **파일**: `.maestro/SCENARIOS.md`(S9~S15) + yaml 6개, spec `0017`.
- **게이트**: ✅ ALL PASS(코드 무변경).
- **maestro 실측(2026-09-22, iPhone 17 Pro+로컬 API/Postgres, 카카오 로그인 수동)**: S9~S12 **maestro 4/4 PASS**(S12 iOS 엣지 스와이프 back 성공), S13~S14 **피커 수동으로 실측 PASS**(추가→"1페이지"·삭제→빈 상태), S15 문서만.
- **왜/다음**: 커버 확대. 다음 = 실기능(#15 presign) / 세션 하드닝. 서비스(docker·API·Metro·시뮬)는 켜둔 상태 — 필요시 정리.
