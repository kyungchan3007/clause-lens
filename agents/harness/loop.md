...
### 시나리오 트리거 — 시나리오 생명주기 = 기능 생명주기

"매 태스크"를 아래 트리거로 구체화한다. **기능이 바뀌면 시나리오도 같은 태스크 안에서 따라 바뀐다** — 안 그러면 시나리오가 코드와 어긋나 썩는다(assert가 깨지거나 헛통과).

| 기능 변화 | 시나리오 조치 |
| --- | --- |
| **사용자 가시 화면/흐름 추가** | 시나리오 **추가**(SCENARIOS.md + 가능하면 yaml) |
| **기존 흐름 변경**(문구·단계·조건) | 해당 시나리오 **업데이트**(assert 문자열/스텝 동기화) |
| **기능/화면 삭제** | 시나리오 + yaml **삭제**(죽은 시나리오 제거) |
| **순수 백엔드/로직/chore** | e2e 해당 없음 → **단위 테스트만** |
...
- **PR 머지·이슈 종료 시점에도 즉시 갱신한다.** 이슈의 "완료 조건"과 연결 spec의 Acceptance를 **묻기 전에** 실제 상태로 채운다 — `Closes #NN`로 이슈가 닫혀도 체크박스가 미체크로 남으면 "완료/진행중"이 안 보인다. (닫힘 = 자동 완료 아님, 체크는 사람이 반영.)
...
- [ ] (코드 태스크면) **단위 테스트 + e2e 테스트**를 작성·실행했나? (§테스트 필수)
- [ ] **시나리오 트리거** — 이번 변경이 건드린 화면 흐름의 시나리오를 **추가/수정/삭제**로 동기화했나? UI 문구 변경 시 yaml assert도 맞췄나? (§테스트 필수 트리거 표)
